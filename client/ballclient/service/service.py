...
import ballclient.service.ai_think as ai
...
def leg_start(msg):
    ...
    AI.set_map_tunnel(msg['msg_data']['map']['tunnel'])
    
    print "wormhole:%s" % msg['msg_data']['map']['wormhole']
    AI.set_map_wormhole(msg['msg_data']['map']['wormhole'])
    
    print "teams:%s" % msg['msg_data']['teams']
    AI.set_team(msg['msg_data']['teams'])
...
def round(msg):
    ...
    temp=AI.on_round(msg['msg_data'])
    #这里返回的temp应该是一个map,  player_id:'up'
